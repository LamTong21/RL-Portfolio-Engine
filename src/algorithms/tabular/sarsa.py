...
class SARSAAgent:
    """Tabular SARSA Agent implemented in NumPy."""
    ...
    def select_action(self, state: int, evaluate: bool = False) -> int:
        """Selects an action using an epsilon-greedy policy with tie-breaking."""
    ...
    def update(
        ...
        """Performs on-policy SARSA update using the actual next action chosen.
...
def train_sarsa(
    ...
    """Convenience pipeline for training SARSA on discrete environments."""
...
if __name__ == "__main__":
    trained_agent, rewards = train_sarsa(num_episodes=300)
    print(f"SARSA training complete. Mean reward (last 50 eps): {np.mean(rewards[-50:]):.2f}")